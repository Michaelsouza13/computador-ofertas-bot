...
SEARCH_URL = "https://www.pichau.com.br/search?q={termo}"
...
class PichauScraper(BaseScraper):
    ...
    def _parse_search(self, html: str, max_offers: int) -> list[Offer]:
        ...
        offers = []
        seen = set()
        products = soup.find_all("div", class_=re.compile(r"product-item|card|MuiPaper"))
        for prod in products:
            ...
            price_el = prod.find(["span", "div", "strong"], class_=re.compile(r"price|boleto|por|Pix"))
            current = 0.0
            if price_el:
                try:
            ...
            offers.append(Offer(
                ...
                current_price=current, product_url=full_url, platform="pichau",
            ))
        return offers
